...
(function() {
  ...
  TouchEvent.prototype.leftTap = function() {
    this.viewer.goPrevPage();
  };
  TouchEvent.prototype.centerTap = function() {
    // do nothing
  };
  TouchEvent.prototype.rightTap = function() {
    this.viewer.goNextPage();
  };
  ...
  var Gesture = function(appEvent) {
    ...
    /* Generate class member `type` from `typeDefinition`. */
    (function(self) {
      const dict = {};
      self.typeDefinition.forEach(function(value, i) {
        dict[value] = i;
      });
      self.type = dict;
    })(this);
  ...
  Gesture.prototype.typeDefinition = [
    "ND",
    "leftTap",
    "centerTap",
    "rightTap",
  ...
  /* This will be auto generated by the class constractor. */
  Gesture.prototype.type = null;
  ...
  Gesture.prototype.pushStart = function(id, x, y) {
    ...
    window.setTimeout(function() {
      console.log("callback of a timer to check whether longTap or not; id: ", id);
  ...
  Gesture.prototype.pushEnd = function(id, x, y) {
    ...
    var type = this._getGestureType(touch.start, end);
    console.log("gestureType: %s, gestureName: %s", type, this.typeDefinition[type]);
    touch.type = type;
  };
  ...
  Gesture.prototype._getGestureType = function(start, end) {
    const swipeType = this._getSwipeType(start, end);
    if (swipeType) {
      return swipeType;
    } else {
      return this._getTapType(end.x);
    }
  };
  Gesture.prototype._getTapType = function(x) {
    const pos = x - window.scrollX;
    const border = screen.width / 2;
    const centerBorder = screen.width * App.centerTapRatio / 2;
    console.log("centerBorder", centerBorder);
    console.log("click x", pos);
    if (pos > border) {
      if (pos > border + centerBorder) {
        return this.type.rightTap;
      } else {
        return this.type.centerTap;
      }
    } else {
      if (pos < border - centerBorder) {
        return this.type.leftTap;
      } else {
        return this.type.centerTap;
      }
    }
  ...
  Gesture.prototype.isLeftTap = function(id) {
    if (id in this.touches) {
      return this.touches[id].type === this.type.leftTap;
    }
  };
  Gesture.prototype.isCenterTap = function(id) {
    if (id in this.touches) {
      return this.touches[id].type === this.type.centerTap;
    }
  };
  Gesture.prototype.isRightTap = function(id) {
    if (id in this.touches) {
      return this.touches[id].type === this.type.rightTap;
    }
  };
  ...
  Dispacher.prototype.dispatchEnd = function(id, x, y) {
    ...
    if (this.gesture.isLeftTap(id)) {
      this.appEvent.leftTap();
    } else if (this.gesture.isCenterTap(id)) {
      this.appEvent.centerTap();
    } else if (this.gesture.isRightTap(id)) {
      this.appEvent.rightTap();
